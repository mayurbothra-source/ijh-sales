...
const USERS = [
  ...
  { id: "bh1",  name: "Rohan Mehta",  role: "biz_head",    title: "Business Head", password: "ijh@1234" },
  { id: "sp1",  name: "Rajesh Kumar", role: "salesperson", title: "Sales Exec",    password: "ijh@1234" },
  { id: "sp2",  name: "Priya Sharma", role: "salesperson", title: "Sales Exec",    password: "ijh@1234" },
  { id: "sp3",  name: "Amit Singh",   role: "salesperson", title: "Sales Exec",    password: "ijh@1234" },
  { id: "sp4",  name: "Neha Patel",   role: "salesperson", title: "Sales Exec",    password: "ijh@1234" },
];

const SEED_ORDERS = [
  { id: "o1", userId: "sp1", customer: "Bright Electricals", city: "Mumbai", region: "West", productType: "SMD 2835", productSubtype: "3V 0.2W", package: "SMD 2835", wattage: "0.2W", colour: "White", voltage: "3V", qty: 50000, value: 125000, month: 0, year: 2026, date: "2026-01-05", notes: "Repeat order" },
  { id: "o2", userId: "sp2", customer: "Shine LED Solutions", city: "Delhi", region: "North", productType: "COB LEDs", productSubtype: "20W", package: "COB", wattage: "20W", colour: "White", voltage: "36V", qty: 2000, value: 180000, month: 0, year: 2026, date: "2026-01-12", notes: "" },
  { id: "o3", userId: "sp1", customer: "Lakshmi Traders", city: "Pune", region: "West", productType: "SMD 5050", productSubtype: "RGB", package: "SMD 5050", wattage: "0.2W", colour: "RGBP", voltage: "3V", qty: 10000, value: 95000, month: 1, year: 2026, date: "2026-02-03", notes: "New customer" },
  { id: "o4", userId: "sp3", customer: "National Lighting Co.", city: "Bangalore", region: "South", productType: "LED Modules", productSubtype: "Round", package: "LED Module", wattage: "10W", colour: "White", voltage: "9V", qty: 5000, value: 220000, month: 1, year: 2026, date: "2026-02-18", notes: "" },
  { id: "o5", userId: "sp4", customer: "Glow Tech Pvt Ltd", city: "Chennai", region: "South", productType: "DIP LED M5", productSubtype: "White", package: "DIP M5", wattage: "0.1W", colour: "White", voltage: "6V", qty: 100000, value: 80000, month: 2, year: 2026, date: "2026-03-02", notes: "Price negotiated" },
  { id: "o6", userId: "sp2", customer: "Star Electricals", city: "Kolkata", region: "East", productType: "SMD 3030", productSubtype: "High Lumen", package: "SMD 3030", wattage: "2W", colour: "White", voltage: "18V", qty: 8000, value: 144000, month: 2, year: 2026, date: "2026-03-10", notes: "" },
];

const SEED_VISITS = [
  { id: "v1", userId: "sp1", customer: "Bright Electricals", city: "Mumbai", lat: 19.0760, lng: 72.8777, date: "2026-01-05", notes: "Discussed Q1 requirements", orderId: "o1" },
  { id: "v2", userId: "sp2", customer: "Shine LED Solutions", city: "Delhi", lat: 28.7041, lng: 77.1025, date: "2026-01-12", notes: "Presented new COB range", orderId: "o2" },
  { id: "v3", userId: "sp3", customer: "National Lighting Co.", city: "Bangalore", lat: 12.9716, lng: 77.5946, date: "2026-02-18", notes: "Signed module agreement", orderId: "o4" },
];
...
function useStorage(key, init) {
  const [val, setVal] = useState(() => {
    try {
      ...
      // Migrate mixParams: if it's missing the flat wattages array, rebuild from defaults
      if (key === "ij_mixparams") {
        return {
          ...DEFAULT_MIX_PARAMS,
          ...parsed,
          wattages: Array.isArray(parsed.wattages) && parsed.wattages.length > 0
            ? parsed.wattages
            : DEFAULT_MIX_PARAMS.wattages,
          packages: Array.isArray(parsed.packages) && parsed.packages.length > 0
            ? parsed.packages
            : DEFAULT_MIX_PARAMS.packages,
          colours: Array.isArray(parsed.colours) && parsed.colours.length > 0
            ? parsed.colours
            : DEFAULT_MIX_PARAMS.colours,
          voltages: Array.isArray(parsed.voltages) && parsed.voltages.length > 0
            ? parsed.voltages
            : DEFAULT_MIX_PARAMS.voltages,
        };
      }
      return parsed;
    } catch { return init; }
...
export default function App() {
  const [orders, setOrders] = useStorage("ij_orders", SEED_ORDERS);
  const [visits, setVisits] = useStorage("ij_visits", SEED_VISITS);
  const [targets, setTargets] = useStorage("ij_targets", SEED_TARGETS);
  const [mixParams, setMixParams] = useStorage("ij_mixparams", DEFAULT_MIX_PARAMS);
  const [users, setUsers] = useStorage("ij_users", USERS);
  const [currentUser, setCurrentUser] = useState(null);
  const [page, setPage] = useState("dashboard");

  // Keep currentUser name in sync if it was edited
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0f172a", fontFamily: "'DM Sans', sans-serif", overflow: "hidden" }}>
      ...
      <Sidebar currentUser={syncedCurrentUser} page={page} setPage={setPage} onLogout={() => setCurrentUser(null)} users={users} setUsers={setUsers} />
      <main style={{ flex: 1, overflowY: "auto", padding: "28px 32px" }}>
        ...
        {page === "orders" && <Orders orders={orders} setOrders={setOrders} currentUser={syncedCurrentUser} mixParams={mixParams} users={users} />}
        {page === "visits" && <Visits visits={visits} setVisits={setVisits} orders={orders} currentUser={syncedCurrentUser} users={users} />}
        {page === "targets" && ["admin","biz_head"].includes(syncedCurrentUser.role) && <Targets orders={orders} targets={targets} setTargets={setTargets} currentUser={syncedCurrentUser} users={users} setUsers={setUsers} />}
      </main>
    </div>
  );
}
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #0f172a 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.04)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: "48px 40px", width: 380, boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ marginTop: 24, padding: "16px", background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)" }}>
          <p style={{ color: "#64748b", fontSize: 11, margin: "0 0 8px", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.8 }}>Demo Accounts</p>
          {users.map(u => <p key={u.id} style={{ color: "#94a3b8", fontSize: 12, margin: "2px 0", fontFamily: "DM Mono" }}>{u.name} / {u.password} <span style={{ color: "#475569" }}>({u.title})</span></p>)}
        </div>
      </div>
    </div>
  );
...
function Orders({ orders, setOrders, currentUser, mixParams, users }) {
  const isManager = ["admin","biz_head"].includes(currentUser.role);
  const myOrders = isManager ? orders : orders.filter(o => o.userId === currentUser.id);
  ...
  const handleSaveOrder = (newItems) => {
    setOrders(prev => [...prev, ...newItems.map(it => ({ ...it, userId: currentUser.id }))]);
    setShowForm(false);
  };

  const handleEditSave = (updatedItems) => {
    const updatedIds = new Set(updatedItems.map(it => it.id));
    const oldIds = new Set(editingPO.map(it => it.id));
    setOrders(prev => [
      ...prev.filter(o => !oldIds.has(o.id)),
      ...updatedItems,
    ]);
    setEditingPO(null);
  };
  ...
  const handleDeletePO = (poItems) => {
    const ids = new Set(poItems.map(o => o.id));
...
function Visits({ visits, setVisits, orders, currentUser, users }) {
  const isManager = ["admin","biz_head"].includes(currentUser.role);
  const myVisits = isManager ? visits : visits.filter(v => v.userId === currentUser.id);
  const [showForm, setShowForm] = useState(false);
  ...
  const saveVisit = () => {
    ...
    const now = new Date();
    setVisits([...visits, { ...form, id: `v${Date.now()}`, userId: currentUser.id, date: now.toISOString().split("T")[0], photos: photos.map(p => p.data) }]);
    setShowForm(false);
    resetForm();
  };
...
function Targets({ orders, targets, setTargets, currentUser, users, setUsers }) {
  const now = new Date();
  ...
  const saveEdit = (id) => {
    ...
    if (!isNaN(val) && val >= 0) setTargets(prev => ({ ...prev, [id]: { monthly: val } }));
    setEditingId(null);
  };
